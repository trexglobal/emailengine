...
function init(args) {
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            const providerOrder = { gmail: 0, gmailService: 0, outlook: 1, outlookService: 1 };
            oauth2apps.sort((a, b) => {
                let aOrder = providerOrder[a.provider] ?? providerOrder[a.providerData?.provider] ?? 2;
                let bOrder = providerOrder[b.provider] ?? providerOrder[b.providerData?.provider] ?? 2;
